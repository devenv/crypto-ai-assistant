...
def test_calculate_and_display_indicators(indicator_service: IndicatorService, mock_client: MagicMock, caplog: LogCaptureFixture) -> None:
    """
    Test that indicator calculation runs and logs output.
    ...
    assert "--- Latest Technical Indicators for BTCUSDT ---" in log_output
    assert "Close" in log_output
    assert "RSI" in log_output
    assert "EMA 10" in log_output
    assert "MACD Line" in log_output
    assert "Support Levels" in log_output


def test_calculate_and_display_handles_none_indicators(
    indicator_service: IndicatorService, mock_client: MagicMock, mock_config: AppConfig, caplog: LogCaptureFixture
) -> None:
    """Test that N/A is handled correctly in logging when indicator calculations fail."""
    import logging
    ...
    log_output = caplog.text
    assert "--- Latest Technical Indicators for BTCUSDT ---" in log_output
    assert "RSI            : N/A" in log_output
    assert "MACD Line      : N/A" in log_output


def test_insufficient_data_for_indicators(indicator_service: IndicatorService, mock_client: MagicMock, caplog: LogCaptureFixture) -> None:
    """Test that warnings are logged when there is not enough data."""
    import logging
    ...
    assert "Insufficient data points (10) for FEWUSDT" in caplog.text

    # Test with no data
    mock_client.get_klines.return_value = []
    indicator_service.calculate_and_display_indicators(coin_symbols=["NONE"])
    assert "No kline data fetched for NONEUSDT" in caplog.text
